...
## 3. Audit table (per citation)

One row per (in-text citation × cited work). Columns:

| column | source | notes |
|--------|--------|-------|
| `citation_id` | script | stable id |
| `marker` | script | rendered in-text marker, e.g. `16` or `8-10` |
| `section` | script | paragraph style / section |
| `cited_sentence` | script | the sentence the marker sits in |
| `authors`, `title`, `year`, `journal` | script | authoritative metadata |
| `doi`, `pmid`, `url` | script | hyperlinked in xlsx |
| `existence_status` | script | e.g. `verified (DOI+PMID)` / `unresolved` |
| `match_confidence` | script | high (embedded id, verified) → low |
| `summary` | **model** | 1–2 sentences on the reference's main point |
| `connection` | **model** | one sentence linking reference to the sentence |
| `appropriateness` | **model** | `Supports` / `Partial` / `Unclear` / `Mismatch` |
| `flag` | script+model | TRUE if any flag rule fires |
| `flag_reason` | script+model | why it was flagged |
| `notes` | either | free text |

## 4. Library table (per work)

One row per unique work: authors, title, year, journal, volume, issue, pages,
DOI, PMID, URL, existence status, citation count, summary, notes. This is the
human-readable companion to the exported library files.
...
## 7. Library export formats (generated by default)
